...
class main(object):
    def __init__(self):
        ...
        if sys.argv[1] == 'train':
            ...
            strategy = Strategy.Strategy(symbol=sys.argv[2],binance_client=self.client,filename=sys.argv[3],learning_rate=learning_rate)
        elif sys.argv[1] == 'create':
            #from 4th index we have the config of the network
            config = [int(sys.argv[i]) for i in range(4,len(sys.argv))]
            model = Model.Model()
